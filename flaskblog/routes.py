...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash('Dang ky thanh cong! Hay tien hanh dang nhap!','success')
...
def save_picture(form_picture):
    radom_hex = secrets.token_hex(8)
    _, f_ext = os.path.splitext(form_picture.filename)
    picture_fn = radom_hex +f_ext
...
def post(post_id):
    ...
    print(form.errors)
    if form.validate_on_submit():
        comment = Comment(idpost=post_id,body=form.body.data,
        author=current_user)
